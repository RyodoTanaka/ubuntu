# About
`ros-noetic-desktop-full` + `ros-foxy-desktop` environment.  

# Build
### Download via DockerHub
```bash
$ docker pull ryodo/ubuntu:focal-noetic-foxy
```
### Build by DockerFile
```bash
$ docker build -t ryodo/ubuntu:focal-noetic-foxy .
```
...
```bash
docker run -p 6080:80 -p 5900:5900 -e RESOLUTION=1920x1080 -v /dev/shm:/dev/shm -v /media:/media -v ~/docker:/home/ubuntu/docker ryodo/ubuntu:focal-noetic-foxy
```
### 2. Launch GUI client
